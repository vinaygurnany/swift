...
class AccessScope {
  ...
  /// scope limitation.
  /// If the declaration context is set, and the limit kind is Private, the access
  /// level is considered 'private'. Whether it's 'internal' or 'fileprivate' is determined
  /// by what the declaration context casts to.
  /// If the declaration context is null, and the limit kind is None, the access
  /// level is considered 'public'. If the limit kind is Private, the access level is
  /// considered SPI. If it's Package, the access level is considered 'package'.
...
public:
  ...
  bool isChildOf(AccessScope AS) const {
    ...
    // If this is public, it can't be less than access level of AS, so return false
    return false;
  }

  bool isInternalOrLess() const {
    return getDeclContext() != nullptr;
  }
